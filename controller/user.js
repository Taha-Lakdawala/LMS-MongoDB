...
exports.postIssueBook = async (req, res) => {
    ...
    const { errors, isValid } = validateBookInput.findBook(req.body);
    // Check validation
    if (!isValid) {
        return res.status(400).json(errors);
    }
    // Fetch the user
    const user = await User.findOne({ _id: userId }).catch(err => console.log(err));
...
exports.getUserHistory = async (req, res) => {
    ...
    const issue = await Issue.find({user:userId}).catch(err => console.log(err));
...
exports.putUpdateUser = async (req,res) =>{
    ...
    const user = await User.findOne({ _id: userId }).catch(err => console.log(err));
    if (email){
        const user_with_provided_mail = await User.findOne({ email: req.body.email }).catch(err => console.log(err));
        if (user_with_provided_mail){
            return res.status(409).json({ email: "User with this email already exists!" });
        }
        else{
            const { errors, isValid } = validateAuthInput.isEmail({email});
    ...
    if (password){
        const { errors, isValid } = validateAuthInput.isPassword({password,password2});
        // Check email validation
        if (!isValid) {
            return res.status(400).json(errors);
        }
